...
const { getRecords, doSomething} = proxyActivities<typeof activities>({
...
export async function processBatch(batch: Batch, previousExecutionResult?: Result): Promise<Result> {
  ...
  const map = records.map(async (record) => {

    const jobParams = await doSomething();

    return startChild(recordProcessor, {
      workflowId: workflowInfo().workflowId + '/child-' + record.id+ "/"+ jobParams,
      args: [record],
    });
