...
public class PrepareTestFiles {
    ...
    private static void generateAndSaveHistoriesToFolder(final Path path) {
        ...
        client.listExecutions("").forEach(f -> {
            ...
            final GetWorkflowExecutionHistoryRequest getWorkflowExecutionHistoryRequest =
                    GetWorkflowExecutionHistoryRequest.newBuilder()
                            ...
                            .build();
            final History history = client.getWorkflowServiceStubs()
                    .blockingStub()
                    .getWorkflowExecutionHistory(getWorkflowExecutionHistoryRequest).getHistory();

            String json = new WorkflowExecutionHistory(history).toJson(true);

            try {
                Files.writeString(Path.of(path.toString(), f.getWorkflowExecutionInfo().getExecution().getRunId() + ".json"), json);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
        });
