...
with workflow.unsafe.imports_passed_through():
    ...
    from .workflow import ExecuteActivityResult, SandboxWorkflow
...
class Sandbox:
    ...
    async def run_activity(
        self, sleep_time_seconds: int, *, disable_auto_resume: bool = False
    ) -> ExecuteActivityResult:
        return await workflow.execute_activity(
            "send-sandbox-run-activity",
            dispatch.SendExecuteActivityInput(
                self._id, str(workflow.uuid4()), sleep_time_seconds, disable_auto_resume
            ),
            start_to_close_timeout=DISPATCH_ACTIVITY_TIMEOUT,
