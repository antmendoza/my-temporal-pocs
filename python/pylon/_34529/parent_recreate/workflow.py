"""ParentRecreateWorkflow — the "replace-and-restore" variant of self-healing.

Contrast with the `self_healing` example: there the child `SandboxWorkflow` catches
the in-VM activity timeout and re-provisions a worker *in place* (same sandbox, same
task queue). Here the child does NOT self-heal — the timeout propagates out of the
child and surfaces in this parent, which tears the dead sandbox down and creates a
...
mirrors real AgentCore, where a re-provision is a genuinely new micro-VM and state
...
with workflow.unsafe.imports_passed_through():
    from sandbox_replace.compute import NO_IDLE_TIMEOUT, ProviderDetails
    from sandbox_replace.providers.mockAgentCore import PROVIDER_MOCK_AGENTCORE
    from sandbox_replace.sandbox import new_sandbox
